	...
	for (int i = 0; i < scene->current_targets; i++)
		if (scene->targets[i].texture_type == -1)
		{
			...
			else
			{
				...
				{
				case 1:
					scene->targets[i].point = 1;
					scene->targets[i].size = 0.7;
					break;
				case 2:
					scene->targets[i].point = 3;
					scene->targets[i].size = 0.35;
					break;
				case 3:
					scene->targets[i].point = 5;
					scene->targets[i].size = 0.12;
					break;
				}
			}
		...
		{
			if (update_target_motion(i, scene, time))
				return true;
		}
	return false;
}
...
bool checktarget(Camera *camera, Scene *scene)
{
	...
	for (int i = 0; i < scene->current_targets; i++)
	{
		...
		if (discriminant < 0)
			continue;
		if (scene->targets[i].isenemy)
			return true;
		scene->points += scene->targets[i].point;
		scene->targets[i].texture_type = -1;
	}
	scene->current_targets = (scene->points / scene->difficulty_increase) + 1;
	if (scene->current_targets > scene->max_targets)
		scene->current_targets = scene->max_targets;
	if (scene->points > scene->difficulty_increase)
		scene->enemy_chance = 10.0;
	if (scene->points > scene->difficulty_increase * 2)
		scene->enemy_chance = 30.0;
	return false;
}
